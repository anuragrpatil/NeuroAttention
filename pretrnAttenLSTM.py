...
def tokenize(lang):
  lang_tokenizer = tf.keras.preprocessing.text.Tokenizer(num_words= 10000,oov_token="<unk>", filters='')
  ...
  lang_tokenizer.word_index = {e:i for e,i in eng1000.vindex.items()}
  lang_tokenizer.word_index[lang_tokenizer.oov_token] = vocab + 1
  lang_tokenizer.word_index['<start>'] = vocab+2
  lang_tokenizer.word_index['<end>'] = vocab+3
...
if __name__ == "__main__":
    ...
    with open('allsubs-20120423.txt', 'r') as f:
        for batch in keys[0:2]:
            i = 0
            for line in tqdm(f):
                if i<5000000:
                    words =line.split()
                    pretrn[batch]['inputs'].append(['<start>']+words[:20]+['<end>'])
                    pretrn[batch]['targets'].append(['<start>']+words[1:19]+['<end>'])
                else:
                    break
                i+=1
    ...
    for batch in keys[0:2]:
        ...
        inputs = pretrn[batch]['inputs']
        targets = pretrn[batch]['targets']
        ...
        # vocab_inp_size = vocab+1
        # vocab_tar_size = vocab+1
